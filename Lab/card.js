(function() {
    ...
    // ---------------------- New Code to Fix Z-index Issue -----------------------
    document.addEventListener('DOMContentLoaded', () => {
        const cards = document.querySelectorAll('.card');

        // Intersection Observer to detect when the card is in the viewport
        const observer = new IntersectionObserver(entries => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    entry.target.style.zIndex = 10; // Set higher z-index when visible
                } else {
                    entry.target.style.zIndex = 1; // Reset z-index when not in view
                }
            });
